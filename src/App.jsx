...
export default function App() {
  ...
  const Dashboard = () => {
    ...
    // --- DATA CHART GRAFIK KPI (6 BULAN TERAKHIR) ---
    ...
    const kpiChartData = last6Months.map(month => {
        const mFailures = breakdowns.filter(b => availableMachines.find(m => m.id === b.machineId) && filterByDate(b.date, month, false)).length;
        const mPM = pmSchedules.filter(s => availableMachines.find(m => m.id === s.machineId) && filterByDate(s.date, month, false) && (s.status === 'Selesai' || s.status === 'Terverifikasi')).length;
        return { month, mFailures, mPM };
    });
    
    const maxChartVal = Math.max(...kpiChartData.map(d => Math.max(d.mFailures, d.mPM)), 1);
      ...
      <div className="space-y-6">
        ...
        {viewMode === 'overview' && (
          <div className="space-y-6 animate-[fadeIn_0.3s]">
            ...
            {/* Fitur Baru: Chart Grafik KPI */}
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 w-full">
               <h3 className="font-bold text-gray-800 mb-4 border-b pb-2"><i className="fa-solid fa-chart-column mr-2 text-blue-500"></i> Tren Grafik KPI (6 Bulan Terakhir)</h3>
               <div className="flex items-end h-48 gap-2 md:gap-4 w-full pt-4">
                  {kpiChartData.map((d, i) => {
                      const failHeight = (d.mFailures / maxChartVal) * 100;
                      const pmHeight = (d.mPM / maxChartVal) * 100;
                      return (
                          <div key={i} className="flex-1 flex flex-col justify-end items-center group relative h-full">
                              <div className="flex gap-1 items-end w-full justify-center h-full">
                                  <div className="w-1/3 max-w-[30px] bg-red-400 hover:bg-red-500 rounded-t-sm transition-all relative" style={{ height: `${failHeight}%`, minHeight: d.mFailures > 0 ? '10%' : '0' }}>
                                     <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-bold text-red-600 opacity-0 group-hover:opacity-100">{d.mFailures}</span>
                                  </div>
                                  <div className="w-1/3 max-w-[30px] bg-blue-400 hover:bg-blue-500 rounded-t-sm transition-all relative" style={{ height: `${pmHeight}%`, minHeight: d.mPM > 0 ? '10%' : '0' }}>
                                     <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-bold text-blue-600 opacity-0 group-hover:opacity-100">{d.mPM}</span>
                                  </div>
                              </div>
                              <span className="text-[9px] md:text-xs font-bold text-gray-500 mt-2">{d.month}</span>
                          </div>
                      )
                  })}
               </div>
               <div className="flex justify-center gap-4 mt-4 text-xs font-bold">
                  <div className="flex items-center text-red-600"><span className="w-3 h-3 bg-red-400 rounded-full mr-1"></span> Breakdown Mesin</div>
                  <div className="flex items-center text-blue-600"><span className="w-3 h-3 bg-blue-400 rounded-full mr-1"></span> PM Selesai</div>
               </div>
            </div>
  ...
  const CekHarian = () => {
    ...
    // Fitur perbaikan Bug (defensive programming)
    const availableMachines = getAvailableMachines() || [];
              ...
              <table className="w-full border-collapse border border-gray-900 text-[10px] print:text-[10px]">
                ...
                <tbody>
                  {filteredChecks.length > 0 ? filteredChecks.map(check => {
                     ...
                     const tglParts = check.date ? check.date.split('-') : [];
                   ...
                   <div className="max-h-80 overflow-y-auto">
                      {notifications.length > 0 ? notifications.map((n, idx) => (
                         <div key={idx} onClick={() => { 
                             ...
                             setActiveMenu(n.action); 
                             setIsNotifOpen(false); 
                           }} 
                           className="p-3 border-b border-gray-100 hover:bg-blue-50 cursor-pointer flex gap-3 items-start transition-colors relative pr-8">
                            ...
                            <button onClick={(e) => { 
                              ...
                              className="absolute right-3 top-3 text-gray-400 hover:text-red-500" title="Tutup">
